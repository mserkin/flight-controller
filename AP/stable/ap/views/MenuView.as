...
//methods
	...
	static private function displayThumbnail(IsBox: Boolean, AnItemNumber: int, AnIconRect: Rect, AParams: Object)
	{
		...
		if (IsBox) {
			...
			{
				if (!airfield.IsRunway) continue;
				var rect_scr: Region = airfield.getRegion().cloneRegion();
				rect_scr.Extent.Width /= dbl_zoom;
				rect_scr.Extent.Height /= dbl_zoom;
				rect_scr.Location.X = (rect_scr.Location.X - rg_model.Location.X) / dbl_zoom 
					+ rect_thumbnail.Location.X + rect_thumbnail.Extent.Width/2;
				rect_scr.Location.Y = (rect_scr.Location.Y - rg_model.Location.Y) / dbl_zoom 
					+ rect_thumbnail.Location.Y + rect_thumbnail.Extent.Height/2;				
				var do_img: DisplayObject = new RunwayThumbnailImage();
				ScreenManager.displayImage(do_img, rect_scr, airfield.ActiveCourse);
			}
		}
	}
