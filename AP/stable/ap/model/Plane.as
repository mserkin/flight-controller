...
//methods
	...
	private function checkRunwayRemainder(AWind: Wind)
	{
			...
			//считаем остаток ВПП
			var dbl_rw_remainder = FTargetAirfield.Length / 2 + obj_dist.Distance * ((Math.abs(ang_diff.Radian) < Math.PI/2) ? +1 : -1);
	...
	public function getCourseAndDistance(AnAirfield: IAirfield, AResult: Object)
	{
		...
		if (Math.abs(ang_diff.Radian) > Math.PI / 2)
			AResult.Course.Radian -= Math.PI;
		AResult.BackCourse.Radian = AResult.Course.Radian - Math.PI;
	}
	...
	private function getCourseToCenterline(): Angle
	{
		...
		var ang_to_centerline: Angle = FTargetAirfield.Course.add(new Angle(Math.PI / 2, Angle.RADIAN));
	...
	private function taxiDownwind()
	{
		...
		{
			nTurn = Course.getRotation(new Angle(FTargetAirfield.ActiveCourse.Radian - Math.PI, Angle.RADIAN));						
		}
		else
		{
	...
	private function taxiToGate(): void
	{
		...
		if (Math.abs(ang_diff.Radian - Math.PI / 2) < DBL_ALLOWABLE_TURN_ERROR)
		{
