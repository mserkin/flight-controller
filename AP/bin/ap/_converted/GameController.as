...
//private fields & consts
	...
	private var dblPreviousSimulationRate: Number;
	...
	private var scChannel: SoundChannel;
...
//methods
	...
	public function processDispatcherEvent(AType: int, AParamObj: Object = null): Boolean
	{
		switch (AType)
		{
			case ControlDispatcher.N_FAST_SIMULATION:
				if (AParamObj.FastMode && (SimulationRate == 1 
					|| SimulationRate == 0 && dblPreviousSimulationRate == 1)) 
				{
					accelerate();
				}
				else if (!AParamObj.FastMode && (SimulationRate > 1 
					|| SimulationRate == 0 && dblPreviousSimulationRate > 1)) 
				{
					decelerate();
				}
			...
			case ControlDispatcher.N_PAUSE_SIMULATION:
				if (AParamObj.PauseMode && (SimulationRate > 0)) {
					pause();
				}
				else if (!AParamObj.PauseMode && (SimulationRate == 0)) {
					resume();
				}			
				return true;
    ...
    {
		var n_frames: int = int(FSimulationRate*2);
	...
	{
		if (FSimulationRate == 0) {
			dblPreviousSimulationRate = Math.min(dblPreviousSimulationRate*N_SIMULSTION_ACCELERATION, DBL_MAX_SIMULATION_RATE);
		}
		else {
	...
	private function controlAircraftQuantity(AnAircraftQuantity: int)
	{
		...
		if (FLandingsDone + AnAircraftQuantity < FLandingsToDo)
		{
			...
			{
				if (Math.random() < apAirport.TrafficIntensity && AnAircraftQuantity < N_MAX_ACTIVE_AIRCRAFT_COUNT)
					emitAircrafts(true); 
			}
		}
	}
	...
	{
		if (FSimulationRate == 0) {
			dblPreviousSimulationRate = Math.max(FSimulationRate/N_SIMULSTION_ACCELERATION, DBL_MIN_SIMULATION_RATE);
		}
		else {
	...
	{
		dblPreviousSimulationRate = FSimulationRate;
		FSimulationRate = 0;
	}
	...
	private function pickAircraftType(CanPickCopter: Boolean = true): AircraftType
	{
		...
		var dbl_copter: Number = apAirport.CopterRate;
		var dbl_prop: Number = apAirport.PropRate;
		...
		else if (dbl_random < dbl_copter + dbl_prop + apAirport.LinerRate)
	...
	{
		FSimulationRate = dblPreviousSimulationRate;
	}
	...
	private function startPlayingMelody(ALevelNumber: int)
	{
		...
		scChannel = snd_melody.play(10000, 0);
		scChannel.soundTransform = transform;
	}
			
	private function stopPlayingMelody()
	{
		if(scChannel) scChannel.stop();
	}			
	...
	private function Aircraft_OnLanded(AnEvent: ObjectEvent):void 
	{
		...
		GameProgress.addPoints(int(aircraft.Fuel*N_FULL_TANK_POINTS));
